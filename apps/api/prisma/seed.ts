...
async function main() {
  ...
  if (existingDeals === 0) {
    ...
    // Competitive-watch list, researched and verified per-entity (see chat
    // history) rather than inferred from platform names — several entries
    // deliberately deviate from the "sounds like fractional" assumption:
    // Tokimo and Livret P are fixed-term bonds tied to real-estate
    // development/marchands de biens, not fractional rental ownership, so
    // they're classified CROWDFUNDING despite operating in the real-estate
    // space. Entries with verificationStatus A_VERIFIER are genuinely
    // unconfirmed as of the last research pass — do not upgrade them to
    // ACTIF without a fresh, sourced check.
    const platformDefs: {
      name: string;
      ...
      },
      {
        ...
        verificationStatus: 'A_VERIFIER',
        verificationNote:
          "Cité sur un forum spécialisé (Investisseurs Heureux) mais aucune confirmation d'activité 2026 fiable.",
      },
      {
        name: 'Fraktion',
        category: 'FRACTIONNE',
      ...
      },
      {
        ...
        verificationStatus: 'ACTIF',
        verificationNote:
          'Pertes en forte hausse (Baromètre Argent & Salaire, avril 2026) — actif mais en difficulté, pas fermé.',
      },
    ];
      ...
      platformDefs.map((p) =>
        prisma.graphEntity.create({
          data: {
            ...
            website: p.website,
            metadata: {
              businessModel: p.category,
              country: p.country,
              verificationStatus: p.verificationStatus,
              verificationNote: p.verificationNote,
              isTerminated: p.verificationStatus === 'LIQUIDE',
            },
          },
        }),
      ),
