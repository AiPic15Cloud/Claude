...
export class AlertsService {
  ...
  create(organizationId: string, data: { title: string; message: string; severity?: AlertSeverity; dealId?: string }) {
    return this.prisma.alert.create({
      data: { organizationId, title: data.title, message: data.message, severity: data.severity, dealId: data.dealId },
    });
  }
