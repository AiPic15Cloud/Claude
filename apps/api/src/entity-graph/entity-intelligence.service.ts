...
/**
 * Requêtes déterministes sur le Knowledge Graph v2 (spec ATLAS v2, B.3) :
 * groupe économique, garanties partagées, sociétés liées à une procédure
 * collective, exposition consolidée. Le groupe économique est désormais
 * traversé transitivement (GroupBuilderService, Market Relationship &
 * Contagion Intelligence V2 §16 "Group Builder") — l'ancienne doctrine
 * "un seul saut, multi-niveaux réservé à B.4/B.5" ne s'applique plus, B.4/B.5
 * étant précisément ce qui vient d'être construit. Séparé de
 * RelationshipsService, qui ne fait que lire/écrire le modèle sans dérivation.
 */
@Injectable()
export class EntityIntelligenceService {
  ...
  async getSummary(organizationId: string, entityId: string): Promise<EntitySummary> {
    ...
    const groupEntityIds = await this.groupBuilder.getGroupEntityIds(organizationId, entityId);
    const groupEconomique =
      groupEntityIds.size === 0
        ? []
        : await this.prisma.entity
            .findMany({ where: { id: { in: [...groupEntityIds] }, organizationId }, select: { id: true, name: true } })
            .then((entities) => entities.map((e) => ({ id: e.id, name: e.name })));
    ...
    // Opérations directes : celles de l'entité elle-même (exposition
    // directe). Opérations consolidées : celles de tout le groupe économique
    // (clôture transitive complète, GroupBuilderService) — plus limité à un
    // seul saut de sœurs.
    ...
    if (groupEntityIds.size > 0) {
      const groupRelationships = await this.prisma.relationship.findMany({
        where: {
          organizationId,
          OR: [{ sourceEntityId: { in: [...groupEntityIds] } }, { targetEntityId: { in: [...groupEntityIds] } }],
        },
        include: {
      ...
      for (const r of groupRelationships) {
