...
import { usePrequalLockBaseline } from '../hooks/use-prequalification';
import type { PrequalBpComparison, PrequalBpComparisonLine } from '@/types';

function formatEuro(value: number): string {
  return new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 }).format(value);
}

function DeltaBadge({ line }: { line: PrequalBpComparisonLine }) {
  if (line.deltaAbs === 0) return <span className="text-xs text-muted-foreground">sans changement</span>;
...
/** Équivalent préqual de BpComparisonCard (dossiers/) — même doctrine (instantané figé explicitement), même présentation, hook dédié au dossier de préqualification. */
export function PrequalBpComparisonCard({ caseId, comparison }: { caseId: string; comparison: PrequalBpComparison }) {
  const lockBaseline = usePrequalLockBaseline(caseId);
